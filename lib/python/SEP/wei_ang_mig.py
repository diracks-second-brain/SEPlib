...
import SEP.pv_copy
import SEP.pv_split
import SEP.wei_ang_op
__author__ = "Robert G. Clapp"
__email__ = "bob@sep.stanford.edu"
__version = "2005.4.10"
                                                                                
class migration(SEP.wei_ang_op.operator):
  def __init__(self,name="ANG_MIG"):
    """Operator to do angle migration"""
    SEP.wei_ang_op.operator.__init__(self,name)
  def add_doc_params(self):
    """Documenation for offset migration"""
    SEP.wei_ang_op.operator.add_doc_params(self)
  ...
  def add_wei_params(self):
    ...
    wei_pars=SEP.wei_ang.base.add_wei_params(self)
  ...
  def add_optional_files(self,parfiles,nsect):
    if self.param("zero_image"):
      parfiles["zero_image"]=SEP.pf_copy(name=self.param("zero_image"),
        tag="zero_image=",dff_axis=5,nblock=[nsect])
    if self.param("wave_out"):
      parfiles["wave_out"]=SEP.pf_copy(name=self.param("wave_out"),
      tag="wave_out=", njobs=nsect,
       dff_axis=5, nblock=[nsect])
    return parfiles
  ...
  def add_image_file(self,parfiles,nsect):
    ...
    if not self.logic("image_real"):
      ...
      p=SEP.pv_copy.float_vector(name=self.param("R"),reuse_par="add=y",
       tag="R=",njobs=nsect, space_only=1)
    parfiles["domain"]=self.add_axes(p,["amx","amy","apx","apy","az_"])
    return parfiles
  ...
  def prep_run(self,restart=None):
     ...
     SEP.wei_ang_op.operator.prep_run(self,restart)
